...
const VOICE_DISPLAY_NAMES: Record<string, string> = {
    "anushka": "Анушка (Женский)",
    "manisha": "Маниша (Женский)",
    "vidya": "Видья (Женский)",
    "arya": "Арья (Женский)",
    "abhilash": "Абхилаш (Мужской)",
    "karun": "Карун (Мужской)",
    "hitesh": "Хитеш (Мужской)",
};
        ...
        return (
            <div className="space-y-6">
                ...
                {currentProvider && providerSchema && providerSchema.properties.api_key && (
                    <div className="space-y-2">
                        <Label>{mode === 'override' ? 'API ключ (оставьте пустым для глобального)' : 'API ключ(и)'}</Label>
            ...
            <div className="flex items-center justify-between mb-4 p-4 border rounded-lg">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                        Использует единую модель речь-в-речь (без отдельного STT/TTS). LLM по-прежнему требуется для извлечения переменных и QA.
                    </p>
                </div>
                <Switch
                    id="realtime-toggle"
