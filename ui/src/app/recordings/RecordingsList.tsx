...
export default function RecordingsList({ refreshKey }: { refreshKey?: number }) {
    ...
    const saveRecordingId = async (rec: RecordingResponseSchema) => {
        ...
        if (!newId) {
            setEditError("ID не может быть пустым");
            return;
        }
        if (!/^[a-zA-Z0-9_-]+$/.test(newId)) {
            setEditError("Только буквы, цифры, дефисы и подчёркивания");
            return;
        }
        ...
        try {
            ...
            toast.success(`ID записи обновлён на "${newId}". Все ссылки в рабочих процессах обновлены.`);
            cancelEditing();
            fetchRecordings();
        } catch (err) {
            setEditError(err instanceof Error ? err.message : "Не удалось обновить ID записи");
        }
    };
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
                <div className="relative flex-1">
                    ...
                    <Input
                        placeholder="Поиск по имени файла, транскрипту или ID..."
                        value={searchQuery}
            ...
            <div className="text-sm text-muted-foreground">
                {filteredRecordings.length} запис{filteredRecordings.length !== 1 ? "ей" : "ь"}
