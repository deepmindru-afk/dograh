...
function formatDuration(seconds?: number | null) {
    if (seconds == null || Number.isNaN(seconds)) return 'Н/Д';
    ...
    return (
        <Card className="border-border">
            <CardHeader className="pb-3">
                <CardTitle className="text-lg">Метрики запуска</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                <MetricCard label="Длительность" value={formatDuration(costInfo?.call_duration_seconds)} />
                <MetricCard
                    label="Использование токенов"
                    value={costInfo?.dograh_token_usage != null ? costInfo.dograh_token_usage.toLocaleString() : 'Н/Д'}
                />
                <MetricCard label="Ходы пользователя" value={String(metrics.userTurns)} />
                <MetricCard label="Ходы бота" value={String(metrics.botTurns)} />
                <MetricCard label="Вызовы инструментов" value={String(metrics.toolCalls)} />
                <MetricCard label="Посещённые узлы" value={String(metrics.visitedNodes)} />
            </CardContent>
        </Card>
    );
}
...
export default function WorkflowRunPage() {
                    ...
                    <Card className="border-border">
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div className="flex items-center gap-4">
                                <CardTitle className="text-2xl">
                                    {isTextChatRun ? 'Текстовая сессия' : 'Запуск агента завершён'}
                                </CardTitle>
                        ...
                        <CardContent>
                            <p className="text-muted-foreground mb-8">
                                {isTextChatRun
                                    ? 'Просмотрите историю разговора, метрики и контекст, собранные для этой текстовой сессии.'
                                    : 'Запуск вашего голосового агента успешно завершён. Вы можете просмотреть или скачать стенограмму и запись.'}
                            </p>
            ...
            <div className="flex h-full items-center justify-center p-6">
                <Card className="w-full max-w-xl border-border">
                    <CardHeader className="space-y-2">
                        <CardTitle className="text-2xl">Детали запуска недоступны</CardTitle>
                        <p className="text-sm text-muted-foreground">
                            Для этого запуска ещё нет страницы с деталями. Вернитесь к workflow, чтобы продолжить тестирование или внести изменения.
                        </p>
                    </CardHeader>
                    <CardFooter>
    ...
    return (
        <WorkflowLayout>
            ...
            {showRunDetailsView && (
                <OnboardingTooltip
                    title='Настройте ваш Workflow'
                    targetRef={customizeButtonRef}
                    message="Отредактируйте ваш workflow, чтобы настроить поведение голосового агента, добавить новые шаги или изменить поток разговора."
