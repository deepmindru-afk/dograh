...
export default function DocumentList({ refreshTrigger }: DocumentListProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-green-500">Завершён</Badge>;
      case 'processing':
        return (
      ...
      {filteredDocuments.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            {searchQuery
              ? 'Нет документов по вашему запросу'
              : 'Документов ещё не загружено'}
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {doc.retrieval_mode === 'full_document' ? (
                      <Badge variant="outline" className="text-xs">Весь документ</Badge>
                    ) : (
                      <Badge variant="outline" className="text-xs">С разбивкой</Badge>
                    )}
                  </div>
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <span>{formatFileSize(doc.file_size_bytes)}</span>
                    {doc.processing_status === 'completed' && doc.retrieval_mode !== 'full_document' && (
                      <span>{doc.total_chunks} чанков</span>
                    )}
                    <span>{formatDate(doc.created_at)}</span>
                  </div>
                  {doc.processing_error && (
                    <p className="text-xs text-destructive mt-1">
                      Ошибка: {doc.processing_error}
                    </p>
                  )}
